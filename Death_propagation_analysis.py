...
class Analysis:
    def __init__(self, file_path, output_path, time_frame, cell_line, treatment, signal_analyzed, n_scramble=1000, draw=False):
        self.file_path = str(file_path.split[0:-1])
...
def create_combined_file(experiments_list, output_path):
    ...
    for exp in experiments_list:
        ...
        mean_time_death.append(exp.mean)
        scramble_signficance_95.append(exp.scrample_95_perc)
        scramble_signficance_98.append(exp.scrample_98_perc)
        scram_mean_time_death.append(exp.scrample_mean)
    ...
    perm_prob = 1 - p_value
    spatial_propagation_index = (scramble_signficance_95 - mean_time_death) / scramble_signficance_95
    data = pd.DataFrame(
        {'name': names,
